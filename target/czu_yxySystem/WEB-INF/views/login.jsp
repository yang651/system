...
<head>
    ...
    <style>
        ...
        .login-main .login-bottom .center .item .icon-1 {
            background: url(${path}/static/images/icon-login.png) no-repeat 1px 0;
        }

        .login-main .login-bottom .center .item .icon-2 {
            background: url(${path}/static/images/icon-login.png) no-repeat -54px 0;
        }

        .login-main .login-bottom .center .item .icon-3 {
            background: url(${path}/static/images/icon-login.png) no-repeat -106px 0;
        }

        .login-main .login-bottom .center .item .icon-4 {
            background: url(${path}/static/images/icon-login.png) no-repeat 0 -43px;
            position: absolute;
            right: -10px;
            cursor: pointer;
        }
        ...
        .login-main .login-bottom .center .item .icon-5 {
            background: url(${path}/static/images/icon-login.png) no-repeat -55px -43px;
        }

        .login-main .login-bottom .center .item .icon-6 {
            background: url(${path}/static/images/icon-login.png) no-repeat 0 -93px;
            position: absolute;
            right: -10px;
            margin-top: 8px;
            cursor: pointer;
        ...
        .login-main .login-bottom .tip .icon-check {
            ...
            height: 14px;
            border: none;
            background: url(${path}/static/images/icon-login.png) no-repeat -111px -48px;
        }
        ...
        body {
            background: url(${path}/static/images/loginbg.png) 0% 0% / cover no-repeat;
            position: static;
            font-size: 12px;
        }
        ...
        <form class="layui-form login-bottom">
            <div class="center">
                <div class="item">
                    ...
                    <input type="text" name="username" id="username" lay-verify="required" lay-reqtext="用户名不可为空"
                           placeholder="请输入用户名" maxlength="24"/>
                </div>

                <div class="item">
                    <span class="icon icon-3"></span>
                    <input type="password" name="password" lay-verify="required" lay-reqtext="密码不可为空"
...
<script>
    layui.use(['form', 'jquery', 'layer'], function () {
        ...
        form.on('submit(login)', function (data) {
            ...
            $.ajax({
                type: 'POST',
                url: '${path}/denglu/yanZheng.do',
                data: {'username': data.username, 'password': data.password},
                success: function (res) {
                    layer.closeAll('loading');
                    if (res.code == 0) {
                        location.href = '${path}/to_main.do';
                    }else if(res.code == 500){
                        var msg = layer.msg(res.msg,{offset: '100px',anim:6,time:false, btn: ['关闭']
                        });
                    }
                },
